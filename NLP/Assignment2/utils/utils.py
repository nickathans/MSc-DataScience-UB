...
def create_corpus(x, y):
    ...
    for sentence, tags in zip(x, y):
        for word, tag in zip(sentence, tags):
            ...
            if tag not in corpus_pos_tag:
                corpus_pos_tag[tag] = b
                b += 1
                
    return corpus_words, corpus_pos_tag
...
def evaluate(model, data, dic_tag_pos, corpus_tag_dic,y_true,save_name,dataframe=None):
    ...
    if dataframe is None:
        ...
        dataframe = pd.DataFrame({"Correct Sentences":[correct_sentence/len(data)],
                         ...
                         "Weighted F1 Score":[sk_f1_score(y_true,y_pred,average="weighted")],
